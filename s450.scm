...
(define (insert! key value table)
  (let ((record (assoc key (cdr record))))
    (if record
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;    Special Form / Action Table
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Many of the functions in the original xeval differing
;; number of arguments, so I have used the lambda's to
;; equalize it.
...
(define (install-special-form tag action)
  (if (symbol? tag)
      (if (procedure? action)
	  (if (equal? (alook tag) #f)
	      (begin (set! stable
		    (cons '*table*
			  (cons (cons tag action)
				(cdr stable))))tag)
	      (error "This tag is already defined.")
	      )
	  (error "Action is not a procedure.")
	  )
      (error "Tag must be quoted")
      )
  )
...
(define (setup-environment)
  (let ((initial-env
         (xtend-environment (primitive-tags)
...
(display "... loaded the metacircular evaluator. (SICPLisp) runs it.")
(newline)
